    ...
    if [[ "$cur" == -* ]]; then
      ...
      case $command in
        @(install|it))
          if [ `ls *.eopkg 2> /dev/null | wc -l` -gt 0 ]; then
            ...
            if `ls /var/lib/eopkg/index/* &>/dev/null`; then
              packages=`grep -P "<Name>[^\s]*</Name>" /var/lib/eopkg/index/*/eopkg-index.xml | gawk -F '>' '{print $2}' | gawk -F '<' '{print $1}' | sort | uniq | tr "\n" " "`
            fi
          fi
          ;;
        @(info))
        ...
        @(upgrade|up|remove|rm))
          if `ls /var/lib/eopkg/index/* &>/dev/null`; then
            packages=`grep -P "<Name>[^\s]*</Name>" /var/lib/eopkg/package/*/metadata.xml | gawk -F '>' '{print $2}' | gawk -F '<' '{print $1}' | sort | uniq | tr "\n" " "`
          fi
          ;;
      esac
